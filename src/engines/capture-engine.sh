...
_capture_init_state() {
    ...
    local current_count
    current_count=$(session_get_metric "frustration_count" "NOT_SET")

    if [[ "${current_count}" == "NOT_SET" ]]; then
...
capture_get_confidence() {
    ...
    # Low confidence for single or few events
    echo "${CONFIDENCE_LOW}"
    return 0
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    frust1=$(capture_detect_event "${EVENT_HANDLER_BLOCKED}" "Bash" "rm_operation")
    echo "Detected: ${frust1}"

    frust2=$(capture_detect_event "${EVENT_HANDLER_ERROR}" "Write" "permission_denied")
    echo "Detected: ${frust2}"

    frust3=$(capture_detect_event "${EVENT_HANDLER_BLOCKED}" "Bash" "rm_operation")
